...
def get_selectDay(request):
    ...
    if request.method == 'POST':
        ...
        current_year = int(request.POST.get('selectedYear'))
        current_month = int(request.POST.get('selectedMonth'))
        current_day = int(request.POST.get('clickedDate'))
